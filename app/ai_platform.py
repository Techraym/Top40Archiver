...
from .ai_operator_chat import router as operator_chat_router
...
def healthz():
    return {
        ...
        "operator_chat": True,
        "operator_chat_route": "/operator-chat",
        "operator_chat_diagnose_mode": True,
        "operator_chat_bounded_repair_mode": True,
        "operator_chat_free_shell": False,
        "operator_chat_lan_only": True,
        ...
        "multi_source_download_engine": True,
        "download_manager_service": "top40-download-manager.service",
        "provider_circuit_breakers": True,
        "provider_search_cache": True,
        "provider_rejected_candidate_memory": True,
        "provider_ai_tuning": True,
        "youtube_primary_download_source": True,
        "youtube_last_resort": False,
        "youtube_max_concurrent": 1,
        "current_chart_download_priority": True,
        "transient_candidate_retry": True,
        "provider_accounts_allowed": False,
        "provider_personal_cookies_allowed": False,
...
def development_dashboard() -> HTMLResponse:
    return HTMLResponse(f"""<!doctype html><html lang='nl'><head><meta charset='utf-8'><meta name='viewport' content='width=device-width,initial-scale=1'><title>Top40 AI Development Assistant</title><style>
:root{{--bg:#f5f2ed;--card:#fff;--ink:#25211e;--muted:#746c64;--line:#e3dad0;--ok:#247a49;--bad:#b52d24}}*{{box-sizing:border-box}}body{{margin:0;background:var(--bg);color:var(--ink);font:15px/1.5 Inter,system-ui,sans-serif}}.shell{{max-width:1300px;margin:auto;padding:28px}}.top{{display:flex;justify-content:space-between;gap:20px;align-items:flex-start}}.badge{{display:inline-block;border:1px solid var(--line);background:#fff;padding:7px 11px;border-radius:999px;color:var(--ink);text-decoration:none}}.nav{{display:flex;gap:8px;flex-wrap:wrap}}.grid{{display:grid;grid-template-columns:repeat(12,1fr);gap:15px;margin-top:22px}}.card{{grid-column:span 4;background:var(--card);border:1px solid var(--line);border-radius:20px;padding:18px}}.wide{{grid-column:span 8}}.full{{grid-column:1/-1}}.muted{{color:var(--muted)}}.ok{{color:var(--ok)}}.bad{{color:var(--bad)}}button,input,textarea{{font:inherit;border:1px solid var(--line);border-radius:12px;padding:11px;background:#fff;color:var(--ink)}}button{{cursor:pointer;background:var(--ink);color:#fff}}textarea,input{{width:100%;margin:5px 0 12px}}textarea{{min-height:110px}}pre{{background:#211e1b;color:#f7f2eb;padding:15px;border-radius:14px;max-height:420px;overflow:auto;white-space:pre-wrap}}.item{{padding:14px 0;border-bottom:1px solid var(--line)}}@media(max-width:800px){{.card,.wide{{grid-column:1/-1}}.top{{flex-direction:column}}}}</style></head><body><main class='shell'><header class='top'><div><span class='badge'>Top40Archiver {VERSION}</span><h1>AI Development Assistant</h1><p class='muted'>Handmatige ontwikkelwerkruimtes blijven sandbox-only. De hoofdpagina op :8041 wordt afzonderlijk door de lokale AI ontworpen via de gevalideerde Control Room UI-loop. De Operator Chat accepteert opdrachten die bijvoorbeeld samen met ChatGPT zijn opgesteld, maar voert uitsluitend evidence-gebonden whitelistacties uit.</p></div><div class='nav'><a href='/' class='badge'>AI Control Room</a><a href='/operator-chat' class='badge'>Qwen Operator Chat</a><a href='/ai-session' class='badge'>Qwen AI Session</a><a href='/ai-learning' class='badge'>AI Learning Center</a><a href='/operations-worker' class='badge'>Operations Worker</a><a href='/ai-actions' class='badge'>AI-herstelactiviteiten</a></div></header><section class='grid'><article class='card wide'><h2>Nieuwe analyse</h2><label>Titel</label><input id='title' placeholder='Bijvoorbeeld: downloader herstelt 429 niet'><label>Probleem</label><textarea id='problem' placeholder='Beschrijf fout, verwacht gedrag en relevante context.'></textarea><button onclick='createWorkspace()'>Werkruimte maken</button><p id='create-result' class='muted'></p></article><article class='card'><h2>Veiligheidsstatus</h2><p class='ok'><b>Sandbox-only handmatige workflow</b></p><p>Geen vrije productiewrites.</p><p>Operator Chat: alleen whitelist + lokale evidence.</p><p>Autonome codepatches alleen na tests/back-up/canary.</p><p>De Control Room HTML/CSS mag lokaal door AI evolueren; runtime-JavaScript blijft policy-code.</p><p>De AI Session/Operator Guidance UI is vaste policy-code en niet zelf-aanpasbaar door Qwen.</p><p>Gedownloade audio blijft buiten bereik.</p></article><article class='card full'><h2>Werkruimtes</h2><div id='items'>Laden…</div></article></section></main><script>
