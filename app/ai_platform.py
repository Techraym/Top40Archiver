...
from .dev_assistant import list_workspaces
...
:root{--bg:#f5f2ed;--card:#fff;--ink:#25211e;--muted:#746c64;--line:#e3dad0;--ok:#247a49;--bad:#b52d24}*{box-sizing:border-box}body{margin:0;background:var(--bg);color:var(--ink);font:15px/1.5 Inter,system-ui,sans-serif}.shell{max-width:1300px;margin:auto;padding:28px}.top{display:flex;justify-content:space-between;gap:20px;align-items:flex-start}.badge{display:inline-block;border:1px solid var(--line);background:#fff;padding:7px 11px;border-radius:999px}.grid{display:grid;grid-template-columns:repeat(12,1fr);gap:15px;margin-top:22px}.card{grid-column:span 4;background:var(--card);border:1px solid var(--line);border-radius:20px;padding:18px;box-shadow:0 12px 30px rgba(50,40,30,.06)}.wide{grid-column:span 8}.full{grid-column:1/-1}.muted{color:var(--muted)}.ok{color:var(--ok)}.bad{color:var(--bad)}button,input,textarea{font:inherit;border:1px solid var(--line);border-radius:12px;padding:11px;background:#fff}button{cursor:pointer;background:var(--ink);color:#fff}textarea,input{width:100%;margin:5px 0 12px}textarea{min-height:110px}pre{background:#211e1b;color:#f7f2eb;padding:15px;border-radius:14px;max-height:420px;overflow:auto;white-space:pre-wrap}.item{padding:14px 0;border-bottom:1px solid var(--line)}@media(max-width:800px){.card,.wide{grid-column:1/-1}.top{flex-direction:column}}</style></head><body><main class='shell'><header class='top'><div><span class='badge'>Top40Archiver 1.16.0</span><h1>AI Development Assistant</h1><p class='muted'>Analyse → sandboxpatch → tests → reviewplan. Productie blijft onaangeraakt.</p></div><a href='/' class='badge'>Operations Center</a></header><section class='grid'><article class='card wide'><h2>Nieuwe analyse</h2><label>Titel</label><input id='title' placeholder='Bijvoorbeeld: downloader herstelt 429 niet'><label>Probleem</label><textarea id='problem' placeholder='Beschrijf fout, verwacht gedrag en relevante context.'></textarea><button onclick='createWorkspace()'>Werkruimte maken</button><p id='create-result' class='muted'></p></article><article class='card'><h2>Veiligheidsstatus</h2><p class='ok'><b>Sandbox-only</b></p><p>Geen directe productiewrites.</p><p>Geen automatische merge.</p><p>PR pas na validatie en review.</p></article><article class='card full'><h2>Werkruimtes</h2><div id='items'>Laden…</div></article></section></main><script>
...
# De Development Assistant-routes worden vóór de bestaande Operations Center-
# router geregistreerd. De bestaande API en hoofdpagina blijven ongewijzigd.
