...
def _operator_acknowledgement(item: dict[str, Any]) -> str:
    fallback = (
        f"Richtlijn #{item['id']} is opgeslagen voor {item['scope']}. "
        + (
            "Nieuwe autonome mutaties in dit domein worden gepauzeerd; monitoring en noodzakelijke canary-verificatie/rollback blijven actief."
            if item["mode"] == "hold"
            else "Ik neem deze correctie vanaf de volgende autonome beoordeling mee binnen de bestaande veiligheidsregels."
        )
    )
    prompt = (
        "Je bent Qwen, de autonome lokale Top40Archiver-assistent. De menselijke operator heeft zojuist een "
        "blijvende richtlijn gegeven. Reageer in maximaal 4 korte Nederlandse zinnen alsof je in een beheerchat "
        "antwoordt. Bevestig wat je hebt begrepen, voor welk domein het geldt en hoe dit jouw volgende autonome "
        "werk beïnvloedt. Geef geen verborgen redeneerstappen, geen shellcommando's en beloof niets buiten de "
        "bestaande veiligheids-/whitelistregels. Een HOLD betekent: geen nieuwe mutaties in dat domein, maar "
        "monitoring en verificatie/rollback van reeds actieve canaries blijven doorgaan.\n\n"
        f"ID={item['id']}\nSCOPE={item['scope']}\nMODE={item['mode']}\nINSTRUCTIE={item['instruction']}"
    )
    try:
...
def acknowledge_guidance(item: dict[str, Any]) -> str:
    message = _operator_acknowledgement(item)
    log_session_event(
        event_type="operator_acknowledged",
        title="Qwen bevestigt operatorrichtlijn",
        message=message,
        domain=str(item.get("scope") or "global"),
        role="assistant",
        status="hold" if item.get("mode") == "hold" else "accepted",
        metadata={"guidance_id": int(item["id"]), "model": MODEL},
    )
    return message
...
def api_create_guidance(payload: GuidanceIn):
    ...
    acknowledgement = acknowledge_guidance(item)
    return {"ok": True, "guidance": item, "acknowledgement": acknowledgement}
