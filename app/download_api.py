...
:root{--bg:#f4f5f7;--card:#fff;--ink:#1f2933;--muted:#67727e;--line:#dfe3e8;--good:#147a48;--warn:#a26700;--bad:#b42318}*{box-sizing:border-box}body{margin:0;background:var(--bg);color:var(--ink);font:14px/1.45 system-ui,sans-serif}.shell{max-width:1500px;margin:auto;padding:24px}header{display:flex;justify-content:space-between;gap:16px;align-items:flex-start;margin-bottom:18px}.nav{display:flex;gap:8px;flex-wrap:wrap}.nav a{color:var(--ink);text-decoration:none;background:#fff;border:1px solid var(--line);padding:8px 11px;border-radius:999px}.metrics,.providers{display:grid;gap:12px}.metrics{grid-template-columns:repeat(5,minmax(0,1fr));margin-bottom:16px}.metric,.provider,.jobs{background:var(--card);border:1px solid var(--line);border-radius:18px;padding:16px}.metric b{font-size:28px;display:block}.metric span{color:var(--muted)}.providers{grid-template-columns:repeat(3,minmax(0,1fr))}.provider h2{margin:0 0 5px}.muted{color:var(--muted)}.good{color:var(--good)}.warn{color:var(--warn)}.bad{color:var(--bad)}button{font:inherit;border:1px solid var(--line);background:#fff;border-radius:10px;padding:7px 10px;cursor:pointer}.row{display:flex;justify-content:space-between;gap:10px;border-top:1px solid var(--line);padding:8px 0}.jobs{margin-top:16px;overflow:auto}table{width:100%;border-collapse:collapse;min-width:800px}th,td{text-align:left;padding:8px;border-bottom:1px solid var(--line);vertical-align:top}@media(max-width:1100px){.metrics{grid-template-columns:repeat(3,1fr)}}@media(max-width:900px){.metrics{grid-template-columns:repeat(2,1fr)}.providers{grid-template-columns:1fr}}@media(max-width:520px){.metrics{grid-template-columns:1fr}.shell{padding:12px}header{flex-direction:column}}</style></head><body><main class='shell'><header><div><h1>Download Providers</h1><p class='muted'>Multi Source Download Engine · directe YouTube is de eerste bron; de overige providers nemen automatisch over als YouTube niet kan leveren.</p></div><div class='nav'><a href='/'>AI Control Room</a><a href='/ai-session'>Qwen AI Session</a></div></header><section class='metrics' id='metrics'></section><section class='providers' id='providers'></section><section class='jobs'><h2>Recente jobs</h2><div id='jobs'>Laden…</div></section></main><script>
...
function providerCard(p){const success=p.success_rate_24h==null?'—':p.success_rate_24h+'%';const cooldown=p.cooldown_until?new Date(p.cooldown_until).toLocaleString():'—';const first=p.provider==='youtube'?' · eerste bron':'';return `<article class='provider'><h2>${esc(p.provider)}</h2><p class='${cls(p.status)}'><b>${esc(p.status)}</b>${first} · health ${esc(p.calculated_health_score)}/100</p><div class='row'><span>Succes 24u</span><b>${esc(success)}</b></div><div class='row'><span>Workers</span><b>${esc(p.active_workers)} / ${esc(p.max_concurrent)}</b></div><div class='row'><span>Prioriteit</span><b>${esc(p.effective_priority)}</b></div><div class='row'><span>Cooldown</span><b>${esc(cooldown)}</b></div><div class='row'><span>Laatste fout</span><span>${esc(p.last_error_category||'—')}</span></div><button onclick="toggle('${esc(p.provider)}',${p.enabled?false:true})">${p.enabled?'Uitschakelen':'Inschakelen'}</button></article>`}
async function load(){try{const d=await api('/api/download/providers');document.getElementById('metrics').innerHTML=`<div class='metric'><b>${esc(d.downloads_24h)}</b><span>Downloads 24u</span></div><div class='metric'><b>${esc(d.without_youtube_24h)}</b><span>Via andere bronnen</span></div><div class='metric'><b>${esc(d.youtube_music_24h)}</b><span>Via YouTube Music</span></div><div class='metric'><b>${esc(d.youtube_24h)}</b><span>Via directe YouTube</span></div><div class='metric'><b class='good'>${esc(d.youtube_share_percent)}%</b><span>YouTube-aandeel</span><small class='muted'>YouTube-family: ${esc(d.youtube_family_share_percent)}%</small></div>`;document.getElementById('providers').innerHTML=(d.providers||[]).map(providerCard).join('');const j=await api('/api/download/jobs?limit=40');document.getElementById('jobs').innerHTML=`<table><thead><tr><th>Track</th><th>Status</th><th>Provider</th><th>Pogingen</th><th>Volgende poging</th><th>Fout</th></tr></thead><tbody>${(j.items||[]).map(x=>`<tr><td>${esc(x.artist)} – ${esc(x.title)}</td><td>${esc(x.status)}</td><td>${esc(x.preferred_provider||'—')}</td><td>${esc(x.attempts)}</td><td>${esc(x.next_attempt_at||'—')}</td><td>${esc(x.error||'')}</td></tr>`).join('')}</tbody></table>`}catch(e){document.getElementById('providers').innerHTML='<article class="provider bad">'+esc(e)+'</article>'}}
