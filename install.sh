...
# De oude periodieke downloadtimer wordt niet meer gebruikt. De permanente
# downloadservice blijft actief en pakt nieuwe wachtrij-items vanzelf op.
systemctl disable --now top40-archiver-download.timer 2>/dev/null || true
...
echo "Automatische updates: twee minuten na opstarten en iedere 24 uur, met commit-SHA- en SHA-256-controle."
...
echo "De permanente downloadservice gebruikt standaard twee parallelle workers."
echo "De downloadservice blijft actief, verwerkt maximaal twintig nummers per batch en start de volgende batch automatisch."
