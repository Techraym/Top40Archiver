...
# Legacy compatibility bootstrap for Top40Archiver 1.16.12.
# The proven 1.16.11 bootstrap is policy-identical; this wrapper renders a
...
[ "$(tr -d '[:space:]' < "$APP/VERSION")" = "1.16.12" ] || {
  echo "FOUT: bootstrap-installer is uitsluitend voor 1.16.12."
  exit 1
}

TMP_RENDERED=$(mktemp /tmp/top40-install-1.16.12.XXXXXX.sh)
cleanup(){ rm -f "$TMP_RENDERED"; }
trap cleanup EXIT

sed \
  -e 's/1\.16\.11/1.16.12/g' \
  -e 's/11611/11612/g' \
  "$BASE" > "$TMP_RENDERED"
