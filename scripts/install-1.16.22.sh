...
# Legacy compatibility bootstrap for Top40Archiver 1.16.22.
...
[ "$(tr -d '[:space:]' < "$APP/VERSION")" = "1.16.22" ] || {
  echo "FOUT: bootstrap-installer is uitsluitend voor 1.16.22."
  exit 1
}
...
TMP_RENDERED=$(mktemp /tmp/top40-install-1.16.22.XXXXXX.sh)
cleanup(){ rm -f "$TMP_RENDERED"; }
trap cleanup EXIT

sed \
  -e 's/1\.16\.11/1.16.22/g' \
  -e 's/11611/11622/g' \
  "$BASE" > "$TMP_RENDERED"
