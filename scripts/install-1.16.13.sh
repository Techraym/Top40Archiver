...
# Legacy compatibility bootstrap for Top40Archiver 1.16.13.
...
[ "$(tr -d '[:space:]' < "$APP/VERSION")" = "1.16.13" ] || {
  echo "FOUT: bootstrap-installer is uitsluitend voor 1.16.13."
  exit 1
}

TMP_RENDERED=$(mktemp /tmp/top40-install-1.16.13.XXXXXX.sh)
cleanup(){ rm -f "$TMP_RENDERED"; }
trap cleanup EXIT

sed \
  -e 's/1\.16\.11/1.16.13/g' \
  -e 's/11611/11613/g' \
  "$BASE" > "$TMP_RENDERED"
