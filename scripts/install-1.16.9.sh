...
TMP=$(mktemp -d /tmp/top40-1169-bootstrap.XXXXXX)
...
payload = {
    ...
    "purpose": "Legacy installation to 1.16.9 verified rollback",
...
for required in \
  ...
  app/providers/base.py; do
  [ -f "$TMP/$required" ] || {
    echo "FOUT: verplichte 1.16.9 module ontbreekt vóór live-swap: $required"
    exit 1
  }
done
...
if [ -f "$LEGACY_AI_DROPIN" ]; then
  ...
  echo "Legacy AI operations-center drop-in verwijderd voor 1.16.9 migratie."
fi

# Oude en eventuele reeds bestaande nieuwe downloader stoppen vóór de live swap.
systemctl stop top40-archiver-download.service top40-download-manager.service 2>/dev/null || true
