...
TMP=$(mktemp -d /tmp/top40-1167-bootstrap.XXXXXX)
...
make_previous_version_backup() {
  ...
  # legacy configuratie; de nieuwe installer heeft die nog niet vervangen.
...
payload = {
    ...
    "purpose": "Legacy installation to 1.16.7 verified rollback",
...
for required in \
  ...
  [ -f "$TMP/$required" ] || {
    echo "FOUT: verplichte 1.16.7 module ontbreekt vóór live-swap: $required"
    exit 1
  }
done
...
# Oude 1.15.x-installaties kunnen een ExecStart-drop-in hebben die de nieuwe
# unified AI-platformunit overschrijft. Verwijder die transactioneel.
if [ -f "$LEGACY_AI_DROPIN" ]; then
  ...
  echo "Legacy AI operations-center drop-in verwijderd voor 1.16.7 migratie."
fi

# Downloaded audio is geen onderdeel van de live codewissel en wordt nooit verwijderd.
systemctl stop top40-archiver-download.service 2>/dev/null || true

TOP40_SOURCE_SHA="$TARGET_SHA" bash "$TMP/update-existing.sh"

# Vervang de oude updater pas na een volledig geslaagde transactionele installatie.
