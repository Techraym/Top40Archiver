...
PYCACHE="/tmp/top40-pycache-${STAMP}"
...
cp -a /etc/systemd/system/top40-archiver-ai.service "$BACKUP/" 2>/dev/null || true

echo "Backup: $BACKUP"

echo "=== Lichte interface opnieuw toepassen ==="
# Een git reset zet de door recovery gegenereerde lichte bestanden terug naar de
# donkere repositorybasis. Pas daarom eerst de bewezen lichte UI-patches toe.
bash scripts/apply-modern-light-ui.sh
bash scripts/apply-light-settings-panel.sh

echo "=== AI-link en coverweergave toevoegen ==="
python3 - <<'PY'
...
text = re.sub(r'/static/style\.css\?v=\d+', '/static/style.css?v=32', text)
text = re.sub(r'/static/live\.js\?v=\d+', '/static/live.js?v=32', text)
...
/* alpha.4 hotfix: passende AI-knop en albumhoezen */
.ai-sidecar-link{display:inline-flex;align-items:center;justify-content:center;text-decoration:none;min-height:46px;padding:0 18px;border-radius:12px;font-weight:700;box-sizing:border-box}
...
'''
if 'alpha.4 hotfix: passende AI-knop en albumhoezen' not in css:
    css += block
style.write_text(css, encoding='utf-8')
PY

echo "=== Syntax controleren zonder app/__pycache__ te schrijven ==="
rm -rf "$PYCACHE"
install -d -o top40archiver -g top40archiver -m 700 "$PYCACHE"
sudo -u top40archiver env PYTHONPYCACHEPREFIX="$PYCACHE" \
  "$APP/venv/bin/python" -m py_compile \
  app/main.py app/db.py app/cover_art.py \
  app/health_engine.py app/health_trends.py app/prediction_engine.py app/ai_sidecar.py
rm -rf "$PYCACHE"

echo "=== AI-sidecar installeren en starten ==="
install -m 0644 systemd/top40-archiver-ai.service /etc/systemd/system/top40-archiver-ai.service
systemctl daemon-reload
systemctl enable top40-archiver-ai.service
systemctl restart top40-archiver-ai.service

echo "=== Hoofdinterface gecontroleerd herstarten ==="
...
echo "=== Coverworker starten ==="
systemctl enable --now top40-archiver-cover-art.timer 2>/dev/null || true
systemctl start top40-archiver-cover-art.service 2>/dev/null || true

printf '%s\n' '1.15.0-alpha.4' > VERSION
chown root:root VERSION
chmod 0644 VERSION

echo "=== Eindstatus ==="
systemctl --no-pager --full status top40-archiver-web.service top40-archiver-ai.service | tail -n 50 || true
echo "KLAAR. Ververs 8040 met Ctrl+F5."
echo "Hoofdpagina: http://$(hostname -I | awk '{print $1}'):8040/"
