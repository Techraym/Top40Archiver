...
TMP=$(mktemp -d /tmp/top40-1168-bootstrap.XXXXXX)
...
payload = {
    ...
    "purpose": "Legacy installation to 1.16.8 verified rollback",
...
for required in \
  ...
  [ -f "$TMP/$required" ] || {
    echo "FOUT: verplichte 1.16.8 module ontbreekt vóór live-swap: $required"
    exit 1
  }
done
...
if [ -f "$LEGACY_AI_DROPIN" ]; then
  ...
  echo "Legacy AI operations-center drop-in verwijderd voor 1.16.8 migratie."
fi
